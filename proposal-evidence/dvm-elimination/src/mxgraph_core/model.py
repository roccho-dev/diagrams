...
def _reduce(events: list[Json]) -> Json:
    ...
    for seq, event in enumerate(events):
        ...
        if op == "diagram.init":
            ...
            else: raise ContractError(f"label target missing: {target}")
...
def build_model(events: list[Json]) -> str:
    ...
    for target, patch in state["positions"].items():
        ...
        if geom is None: raise ContractError(f"position target missing: {target}")
        geom.update({k:v for k,v in patch.items() if k in {"x","y","w","h","lock"}})

    mxfile = ET.Element("mxfile", {"host":"app.diagrams.net", "agent":"jsonl-mxgraph-proof", "version":"direct-current-state-v1"})
...
def command_to_event(command: Json) -> Json:
    ...
    raise ContractError(f"unsupported command: {typ}")
